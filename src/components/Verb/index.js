...
const Verb = (props) => {
	...
	useEffect(
		() => {
			if (randomVerb) {
				...
				setRandomPerson(Object.entries(randomVerb)[Math.floor(Math.random() * 5) + 7]);
	...
	const randomize = () => {
		...
		do {
			setRandomVerb(verbData[Math.floor(Math.random() * verbData.length)]);
			setRandomPerson(Object.entries(randomVerb)[Math.floor(Math.random() * 5) + 7]);
		} while (setRandomPerson[1] === '' || randomVerb.mood_english === 'Imperative Negative');
	};
	const handleRefresh = () => {
		setCorrect(false);
		setRandomVerb({});
	...
	const LevelOne = verbData.filter((verb) => verb.tense_english === 'Present' && verb.mood_english === 'Indicative');

	const Two = verbData.filter((verb) => verb.tense_english === 'Preterite' && verb.mood_english === 'Indicative');
	...
	const Four = verbData.filter(
		(verb) => verb.tense_english === 'Present Perfect' && verb.mood_english === 'Indicative'
	);

	const LevelFour = LevelThree.concat(Four);

	const Five = verbData.filter((verb) => verb.tense_english === 'Future' && verb.mood_english === 'Indicative');
